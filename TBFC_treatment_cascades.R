...
cascade_order_brostrom_spec <- c("no_estimated_pos", "no_tst_placed_on_ltbi", "no_ltbi_diagnosis", "no_rec_ltbi",
                   "no_start_ltbi", "no_complete_ltbi")

cascade_order_long_brostrom_spec <- c("Total TST Positive (estimated)", "Skin Test Placed (estimated)", 
...
cascade_dataset <- tbfc_analysis %>%
  ...
  select(registration_no, tst_read_yn, tst_read_yn, tst_result, 
...
overall_cascade <- cascade_dataset %>%
  summarise(
    ...
    no_complete_ltbi = sum(epi_status == "Completed treatment" | epi_status == "Previous treatment", na.rm = TRUE),
